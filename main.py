...
def test_sensor_reading(adc, pin_out):
    ...
    def test_sensor(pin_on_seconds):
        for pin_off_seconds in pin_off_seconds_list:
            ...
            relative_std_dev = int(stdev(values_list) * 100 / mean or 1)
            print(
                "Pin off s:",
                pin_off_seconds,
                "| Pin on s:",
...
def setup_network():
    ...
    print_debug("Device IP:", device_ip)
...
def loop(mac_addr, adc, pin_out):
    ...
    while True:
        ...
        print_debug("adc percent", adc_percent)
        response = requests.request(
            "POST",
            ENV_VARS["INFLUXDB_URL"],
            data="feuchtigkeit,device_id={} adc_value={}".format(mac_addr, adc_percent),
            headers={"Authorization": "Token {}".format(ENV_VARS["INFLUXDB_TOKEN"])},
        )
